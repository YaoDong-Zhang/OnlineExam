...
public class ExamSubjectMiddleInfoController {
    ...
    public void getExamPaperWithSubject(
        ...
        //------------日志
        System.out.println("查询试卷试题信息 With " + esm);
        List<ExamSubjectMiddleInfo> esms = esmService.findSubject(esm);
    ...
    public void removeSubjectWithExamPaper(
            ...
            HttpServletResponse response) throws IOException {
        //------------日志
        System.out.println("从试卷 " + examPaperId + " 中移除试题 " + subjectId + "，试题分值：" + score);
        ...
        //修改试卷总分
        examPaperInfoService.updateExamPaperScore(map);
        //修改试卷题目数量
        examPaperInfoService.updateExamPaperSubjects(map);
    ...
    public void getChooseSubjectId(
        ...
        if (esmId == null) {
            //------------日志
            System.out.println("需要添加的试题 " + subjectId + " 暂不存在试卷 " + examPaperId + " 中，可进行添加");
            ...
            if (subjectId != null && score != null) {
                ...
                if (ids == null) {
                    ...
                    //------------日志
                    System.out.println("Session 添加试题：是否手动" + handle + ", 试题编号：" + subjectId + ", 试题分数" + score);
                } else {
                    //存在就移除,反之添加
                    if (ids.contains((subjectId + "," + score))) {
                        ids.remove((subjectId + "," + score));
                        //------------日志
                        System.out.println("Session 移除试题：是否手动" + handle + ", 试题编号：" + subjectId + ", 试题分数" + score);
                    } else {
                        ids.add((subjectId + "," + score));
                        //------------日志
                        System.out.println("Session 添加试题：是否手动" + handle + ", 试题编号：" + subjectId + ", 试题分数" + score);
                ...
                //------------日志
                System.out.println("添加试题 " + subjectId + " 到 Session 失败");
                response.getWriter().print("添加失败，试题编号或试题分数异常！");
                return;
            }
        } else {
            //------------日志
            System.out.println("需要添加的试题 " + subjectId + " 已经存在试卷 " + examPaperId + " 中了, 无法进行添加");
    ...
    public void isHandAddSubjectToExamPaper(
        ...
        if (ids != null) {
            ...
            //------------日志
            System.out.println("试题集合为空，不能进行添加试题操作！");
            response.getWriter().print("需要添加的试题为空，操作失败！");
            return;
        }
        //------------日志
        System.out.println("添加试题集合到试卷 " + examPaperId);
        //总分和题目数量信息
        ...
        /** 修改试卷总分 */
        examPaperInfoService.updateExamPaperScore(scoreWithNum);
        /** 修改试卷试题总量 */
        examPaperInfoService.updateExamPaperSubjects(scoreWithNum);

        /** 添加试题到试卷中 */
        esmService.addESM(map);
    ...
    public String isClearChooseSubjectIds(HttpSession session) {
        //------------日志
        System.out.println("清空 Session 中需要添加的试题编号集合");
    ...
    public void isAutoAddSubjectToExamPaper(
        ...
        for (int i = 0; i < subjectSum; i++) {
            ...
            if (indexs.contains(index)) {  //随机索引已经存在
                ...
                //------------日志
                System.out.println("索引 " + index + " 试题编号 " + subjectId + " 成立");
            }
        }
        ...
        //添加
        //------------日志
        System.out.println("添加试题到试卷 " + examPaperId);
        esmService.addESM(addMap);
